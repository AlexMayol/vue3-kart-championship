...
import { Pilot, Race, RaceResult, PilotInRace, Leaderboard } from "@/interfaces";

export const useData = () => {

  const store = new Localit({ domain: 'setup' });
  ...
  for (const data of initialData) {
    for (const race of data.races) {
      ...
      allRaces.push(pilotAndRace)
    }
  }
  // allRaces is a plain array with the id of the pilots, its time in the race and the name of the races.
  // It helps to create the leaderboard
  // console.log(allRaces)

  const races = new Set<string>();

  for (const data of allRaces) {
    races.add(data.name)
  }
  // console.log(races)
  ...
  for (const race of races) {
    ...
    const championship = {
      name: race,
      leaderboard: allRaces.filter((r) => {
        if (r.name === race)
          return { time: r.time, _id: r._id }
      })
    }
  


    championship.leaderboard.sort((a, b) => {
      if (a.time > b.time)
        return 1
      return -1
    })
